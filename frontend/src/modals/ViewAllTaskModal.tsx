import { Add, CheckCircle, Close, Done, Pending, Rule } from '@mui/icons-material'
...
function ViewAllTaskModal(props:any) {
  ...
  const handleInputChange = (e: any) => {
    ...
    const tempTasks = activeProject?.allTasks?.filter((task: any) => {
      ...
      return (
        taskTitle.includes(input) ||
        taskId.includes(input)
      );
    });
    setSearchKey(input);
                  ...
                  <div className='mr-2'>
                    {
                      ...
                      <CheckCircle sx={{fontSize:15,color:colors.SucessGreen,fontWeight:300}}/>:
                      <Pending sx={{fontSize:15,color:colors.highPriority,fontWeight:300}}/>
                    }
                  </div>
                </div>
               </Link> 
